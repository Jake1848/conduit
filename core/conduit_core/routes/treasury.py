...
async def _overview(session: AsyncSession) -> TreasuryOverviewOut:
    ...
    return TreasuryOverviewOut(
        ...
        agent_liabilities_sats=snap.liabilities_sats,
        solvent=snap.solvent,
        solvency_ratio=snap.ratio,
        withdrawable_sats=_withdrawable(
            snap.assets_sats, snap.liabilities_sats, snap.onchain_confirmed_sats, reserve
        ),
...
async def withdraw(
    ...
    if idem_key is not None:
        ...
        if prior is not None:
            ...
            # failed -> nothing was sent; REUSE this row (a fresh insert would
            # collide with its still-present idempotency_key and brick the retry).
            reuse_id = prior.id
    ...
    liabilities = snap.liabilities_sats
    ...
    # A prior FAILED row for this key is re-armed in place (reuse), so its unique
    # key doesn't self-collide.
    if reuse_id is not None:
        wid = reuse_id
        await twd.reset_to_pending(wid, reserve)
    else:
        try:
            ...
            # Only a unique idem_key collision (a concurrent duplicate that won
            # the insert race) is a conflict; other DB errors propagate as 5xx.
            raise IdempotencyConflict(
                "A withdrawal with this Idempotency-Key is already in progress.",
                in_progress=True,
            ) from e
    ...
    try:
        ...
        await twd.mark_failed(wid, str(e))
        log.warning("treasury.withdraw.failed", withdrawal_id=wid, error=str(e))
        raise
